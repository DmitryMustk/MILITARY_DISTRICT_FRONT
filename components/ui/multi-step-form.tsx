        ...
        <div className="relative transition-transform duration-500">
          {steps.map((step, index) => {
            ...
            return animated ? (
              ...
              <NotAnimatedStep key={step.props.name} isActive={isActive}>
                {step}
              </NotAnimatedStep>
            );
          })}
...
interface NotAnimatedStepProps {
  isActive: boolean;
}

function NotAnimatedStep({ isActive, children }: React.PropsWithChildren<NotAnimatedStepProps>) {
  ...
  useEffect(() => {
    if (isActive && stepRef.current) {
      ...
      if (focusableElement) {
        (focusableElement as HTMLElement).scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }
  }, [isActive]);
  ...
  return (
    <div ref={stepRef} className="w-full h-full" aria-hidden={!isActive}>
      {children}
    </div>
  );
}
